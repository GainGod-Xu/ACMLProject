...
class GNN(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, JK="last", drop_ratio=0.2, gnn_type="gin", aggr='add', graph_pooling = "add"):
        ...
        self.x_embedding4 = torch.nn.Linear(1, emb_dim)
    ...
    def forward(self, batch):
        ...
        x = self.x_embedding1(x[:, 0].long()) + self.x_embedding2(x[:, 1].long()) \
            + self.x_embedding3(x[:, 2].long()) + self.x_embedding4(x[:,3].unsqueeze(1))
...
if __name__ == '__main__':
    ...
    os.chdir("/home/yifeiwang/CMRPProject")
    from DatasetModels.GraphDataset import GraphDataset
    from Utils.CMRPConfig import CMRPConfig
    CMRPConfig.graphs_path = "./Datasets/Smiles_Dataset/Smiles_graph/graphs/"

    # Define graph files
    df = pd.read_csv("./Datasets/Smiles_Dataset/Smiles_graph/dataset_4W.csv")
    graph_files = df['graph']
    # Create GraphDataset instance
    dataset = GraphDataset(graph_files, CMRPConfig, data_loading_function='gdata_loading_method2')

    # Create a data loader with custom collate function
    batch_size = 4
    shuffle = True
    data_loader = DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, collate_fn=GraphDataset.collate_fn,
                             drop_last=True)
    ...
    gnn = GNN(3, 32)
    print((gnn(batch)).shape)

    for type in ['gin', 'gcn', 'gat', 'graphsage', 'nnconv']:
        gnn = GNN(3, 32, JK="last", gnn_type=type, aggr='add', graph_pooling="add")
        print(type)
        print((gnn(batch)).shape)

    # test different gnn settings:
    for aggr in ['add', 'mean', 'max']:
        for pool in ["add", "mean", "max", "attention"]:
            for jk in ["last", "max", "sum"]:
                gnn = GNN(3, 32, JK=jk, gnn_type="gin", aggr=aggr, graph_pooling=pool)
                print(aggr + pool + jk)
                print((gnn(batch)).shape)